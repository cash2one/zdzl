...
+ (void)dismissWithAfterDelay:(NSTimeInterval)seconds{
	[[SVProgressHUD sharedView] dismissWithAfterDelay:seconds];
}
...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(self.alpha != 1) {
            ...
            self.hudView.transform = CGAffineTransformScale(self.hudView.transform, 1.8, 1.8);
...
- (UIView *)hudView {
    if(!hudView) {
		...
		hudView.backgroundColor = [UIColor colorWithWhite:0.1 alpha:0.8f];
		hudView.layer.borderColor = whiteColor.CGColor;
		hudView.layer.borderWidth = 2.0f;
		hudView.layer.cornerRadius = 10.f;
