...
- (id) initWithText:(NSString *)text{
	return [self initWithText:text inView:nil];
}
...
- (id) initWithText:(NSString *)text showActivity:(BOOL)activity 
 inPresentationMode:(GCDiscreetNotificationViewPresentationMode)aPresentationMode inView:(UIView *)aView {
    if ((self = [super initWithFrame:CGRectZero])) {
		...
		if(aView==nil){
			aView = [UIApplication sharedApplication].keyWindow.rootViewController.view;
		}
...
- (void) layoutSubviews {
    ...
    [button addTarget:self action:@selector(handleTouchAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (void) placeOnGrid {
	...
	frame.origin.x = roundf((superSize.width-frame.size.width)/2);
...
-(void)handleTouchAction:(id)sender{
