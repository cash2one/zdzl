...
-(void) buildWithMapInfo:(CCTMXMapInfo*)mapInfo{
	...
	//todo tiger
	//[mapInfo.tilesets count]] 包括了碰撞和npc层的问题
	//在做mapInfo.tilesets时候要注意地图图片的问题
	//我在GameOnloadingMap那里强制-2 这是有问题的
	//你到时候根据实际的地图文件来判断要减去多少
	//或者直接约定多出2个层
	
	[GameConnection post:ConnPost_loadMapStart object:[NSNumber numberWithInt:[mapInfo.tilesets count]]];
...
-(id)parseLayer:(CCTMXLayerInfo*)layerInfo map:(CCTMXMapInfo*)mapInfo{
	...
	if(info){
		...
		TiledLayer * layer = [TiledLayer layerWithTilesetInfo:tileset layerInfo:layerInfo mapInfo:mapInfo];
		layerInfo.ownTiles = NO;
		[layer checkContent:[[info objectForKey:@"point"] CGPointValue]];
		return layer;
	}
	return nil;
}
