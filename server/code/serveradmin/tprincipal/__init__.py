...
def name_auth_type():
    name = '.'.join([game_config.SERVER_VER_CONFIG, 'identity.name'])
    auth_type = '.'.join([game_config.SERVER_VER_CONFIG, 'identity.auth_type'])
    return name, auth_type

def session_identity_loader():
    name, auth_type = name_auth_type()
    if name in session and auth_type in session:
...
def session_identity_saver(identity):
    ...
    session[auth_type] = identity.auth_type
