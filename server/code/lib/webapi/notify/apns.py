...
class NotificationMessage(object):
    ...
    def __str__(self):
        ...
        encoded = json.dumps(data, ensure_ascii=0)
        length = len(encoded)
        try:
            return struct.pack("!bIIH32sH%(length)ds" % { "length" : length },
                                 1, self.identifier, self.expiry, 32, self.token, length,encoded)
        except Exception as e:
            logging.error('token:%s, %s, ed:%s, %s', self.token, type(self.token), encoded, type(encoded))
            raise e

class NotificationService(object):
    COUNT = 300 #close every send COUNT message
    ...
    def _stop_send(self, sleep=True):
        if sleep:
            gevent.sleep(5.0)
        conn = self._push_connection
        self._push_connection = None
        if conn:
            conn.close()
    ...
    def _check_send_connection(self):
        try:
            if self._push_connection is None:
                ...
                #addr[0] = '17.172.233.66'#
                addr[0] = "17.149.35.169"
    ...
    def _send_loop(self):
        ...
        try:
            ...
            logging.error('[apns]send error:%s' % traceback.format_exc())
        finally:
            self._send_greenlet = None
            if self.started:
                if msg is not None:
                    self._send_queue.put(msg)
                self._send_greenlet = gevent.spawn_later(5.0, self._send_loop)
    ...
    def _error_loop(self):
        ...
        try:
            while True:
                ...
                logging.info('_error_loop:%s', data)
            ...
            logging.error('[apns]error_loop error:%s' % traceback.format_exc())
        finally:
            self._stop_send(sleep=False)
            self._error_greenlet = None
